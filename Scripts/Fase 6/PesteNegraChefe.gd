...
var tempo_max = rand_range(3,3.5)/main.dificuldade
...
func dispara_corona(delta):
	...
	if intervalo_disparo>=tempo_max and num_ini<4:
		...
		var corona = load("res://Scenes/Inimigos/PesteNegra/PesteNegra.tscn")
		
		corona = corona.instance()
		
		corona.pos_x_peste = rand_range(700,800)
		...
		intervalo_disparo = 0
		tempo_max = rand_range(3,3.5)/main.dificuldade
...
func som_morte():
	get_node("Sons").play("PesteNegraChefeMorte")
