...
var pos_x_peste = rand_range(800,900)

var vida = 3

var intervalo_disparo = rand_range(5.5,6)/main.dificuldade
...
func dispara(delta):
	if get_tree().get_nodes_in_group(main.INIMIGOS).size()<20:
		...
		if intervalo_disparo<=0:
			#som do tiro
			get_node("Sample").play("tiro_peste")
			
			var tiro = load("res://Scenes/Inimigos/PesteNegra/PesteNegra.tscn")
			...
			intervalo_disparo = rand_range(5.5,6)/main.dificuldade
...
func aplica_dano(dano):
	...
	if vida<=0:
		...
		#Som de explosão
		get_node("Sample").play("explosion_peste")
