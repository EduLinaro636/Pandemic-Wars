...
var tempo_max = rand_range(3,5)/main.dificuldade

var vida = 40*main.dificuldade
...
func movimentacao(delta):
	...
	var tamanho = get_node("Sprite").get_item_rect().size.width/2 
	var pos_x = get_pos().x
	
	if(movi.x+tamanho+pos_x<1300 and y==0):
		y = 1
		x = 0
...
func dispara_corona(delta):
	...
	if intervalo_disparo>=tempo_max and num_ini<4:
		...
		corona.pos_x_corona = rand_range(700,800)
...
func som_morte():
	get_node("Sons").play("CoronaChefeMorte")
