...
var pos_x_tuberculose = rand_range(800,900)

var vida = 3

var intervalo_disparo = rand_range(6,6.5)/main.dificuldade
...
func dispara(delta):
	if get_tree().get_nodes_in_group(main.INIMIGOS).size()<20:
		...
		if intervalo_disparo<=0:
			#som do tiro
			get_node("Sample").play("tiro_tuberculose")
			
			var tiro = load("res://Scenes/Inimigos/Tuberculose/Tuberculose.tscn")
			...
			intervalo_disparo = rand_range(6,6.5)/main.dificuldade
...
func _ready():
	...
	if  get_tree().get_nodes_in_group("jogador").size()>0:
			...
			if abs(pos_jogador.x - get_global_pos().x)>400:
				pos_tiro.x = (pos_jogador.x - get_global_pos().x)/950
				pos_tiro.y = (pos_jogador.y - get_global_pos().y)/550
			else:
				pos_tiro.x = (pos_jogador.x - get_global_pos().x)/950
				pos_tiro.y = (pos_jogador.y - get_global_pos().y)/550
...
func movimenta(delta):	
	...
	if pos_atual.x<=pos_x_tuberculose:
		atira = 1
...
func aplica_dano(dano):
	...
	if vida<=0:
		...
		#Som de explosão
		get_node("Sample").play("explosion_tuberculose")
